...
const AnnualResults = () => {
  ...
  // Filter Annual Exams matching session & class
  const annualExams = useMemo(() => {
    return allExams.filter((e) => {
      const isAnnual = e.examType === 'ANNUAL';
      ...
      return isAnnual && sessionMatch && classMatch;
    });
  }, [allExams, selectedSession, selectedClassId]);

  useEffect(() => {
    if (annualExams.length > 0) {
      const match = annualExams.find((e) => e._id === selectedExamId);
      if (!match) setSelectedExamId(annualExams[0]._id);
    } else {
      setSelectedExamId('');
    }
  }, [annualExams]);
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      addToast(err.response?.data?.message || 'Failed to load class annual report', 'error');
      setReport(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs print:hidden">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">Annual Examination Results</h1>
          </div>
          <p className="text-xs text-slate-500 font-medium mt-1 leading-relaxed">
            View annual class matrix performance or generate official final student marksheets for printing and PDF download.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          <div className="grid grid-cols-2 sm:flex sm:items-center gap-2">
            ...
            >
              <Download className={`w-3.5 h-3.5 text-indigo-400 ${downloadingBulkPdf ? 'animate-bounce' : ''} shrink-0`} />
            ...
            <button
              ...
              className="px-3.5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl text-xs font-black flex items-center justify-center gap-1.5 shadow-md active:scale-95 transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-3">
          <div>
            ...
            <select
              ...
              className="w-full h-9 sm:h-10 px-2.5 sm:px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer"
            >
              {sessions.map((s) => (
          ...
          <div className="col-span-2 sm:col-span-1">
            <label className="block text-[10px] sm:text-[11px] font-bold text-slate-500 mb-1">Annual Exam</label>
            <select
              value={selectedExamId}
              onChange={(e) => setSelectedExamId(e.target.value)}
              disabled={annualExams.length === 0}
              className="w-full h-9 sm:h-10 px-2.5 sm:px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-indigo-500 disabled:opacity-50 transition-all cursor-pointer"
            >
              {annualExams.length === 0 ? (
                <option value="">No Annual Exam Found</option>
              ) : (
                annualExams.map((e) => (
      ...
      {activeTab === 'CLASS_MATRIX' && (
        <div className="annual-print-area bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden print:border-none print:shadow-none">
          {report && report.exam && (
            <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
              <div>
                <div className="flex items-center gap-2 text-[10px] sm:text-xs font-extrabold text-indigo-400 uppercase tracking-widest">
                ...
                <h2 className="text-base sm:text-lg font-black text-white mt-0.5 leading-snug">
                  Class: {report.exam.class?.name || 'Class'} Final Annual Result Summary
                </h2>
              ...
              <div className="relative w-full md:max-w-xs print:hidden">
                ...
                <input
                  ...
                  className="w-full h-9 pl-9 pr-3 bg-slate-800 border border-slate-700 text-white placeholder-slate-400 rounded-xl text-xs font-medium focus:outline-none focus:border-indigo-500 transition-all"
                />
              </div>
            </div>
          )}
          ...
          {loading ? (
            <div className="p-12 text-center space-y-3">
              ...
              <p className="text-xs font-extrabold text-slate-600">Loading Annual class results...</p>
            </div>
          ) : !report || !report.subjects ? (
            <div className="p-12 text-center space-y-2 bg-slate-50/30">
              <Award className="w-10 h-10 text-slate-300 mx-auto" />
              <p className="text-sm font-black text-slate-700">Select Class &amp; Annual Exam</p>
                  ...
                  return (
                    <div key={s.studentId} className="p-3.5 space-y-3">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-xl bg-indigo-100 text-indigo-800 font-black text-xs flex items-center justify-center shrink-0 border border-indigo-200">
                            #{s.rank}
                          </span>
                          <div>
                      ...
                      <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 text-center">
                        ...
                        <div>
                          ...
                          <span className="text-xs font-black text-indigo-600">{s.summary.percentage}%</span>
                        </div>
                        <div>
                      ...
                      <div className="pt-1">
                        <button
                          ...
                          className="w-full py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-900 rounded-xl font-extrabold text-xs border border-indigo-200 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                        >
                    ...
                    {filteredStudentRows.map((s) => {
                      ...
                      return (
                        <tr key={s.studentId} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-3 px-3 text-center font-black text-indigo-600 border-r border-slate-200 whitespace-nowrap">
                          ...
                          <td className="py-2 px-3 text-center print:hidden whitespace-nowrap">
                            <button
                              ...
                              className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 rounded-lg font-extrabold text-[11px] border border-indigo-200 flex items-center justify-center gap-1 mx-auto transition-all cursor-pointer"
                            >
      ...
      {activeTab === 'INDIVIDUAL' && (
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4 print:hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
            <h3 className="text-sm font-black text-slate-900">Individual Student Annual Marksheet Directory</h3>
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-9 pl-9 pr-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:border-indigo-500 transition-all"
              />
            </div>
          </div>
            ...
            {filteredStudentRows.map((s) => (
              ...
              >
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-extrabold flex items-center gap-1 cursor-pointer shadow-xs shrink-0"
                >
                  <Eye className="w-3.5 h-3.5" />
                  <span>Marksheet</span>
                </button>
      ...
      <style>{`
        @media print {
          ...
          .annual-print-area, .annual-print-area * {
            visibility: visible;
          }
          .annual-print-area {
            position: absolute;
